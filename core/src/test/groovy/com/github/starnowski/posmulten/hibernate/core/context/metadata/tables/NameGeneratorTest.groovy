...
class NameGeneratorTest extends Specification {
    ...
    @Unroll
    def "should return name with prefix #prefix generated by static method (#generatedString) with passed columns list (#columnNames) and shorten for maximum length : #maxLength, #expectedString" () {
        given:
            def tested = new NameGenerator(maxLength)
            def table = Mock(Table)
    ...
    @Unroll
    def "should return name with prefix #prefix generated by static method (#generatedString) with passed null columns list and shorten for maximum length : #maxLength, #expectedString" () {
        given:
            def tested = new NameGenerator(maxLength)
            def table = Mock(Table)
            generatorMock.when({  -> Constraint.generateName(prefix, table, emptyList()) }).thenReturn(generatedString)
        ...
        when:
            def result = tested.generate(prefix, table, null)
